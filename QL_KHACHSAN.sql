...
FROM PHIEUDATPHONG PDP
	...
	JOIN NHANVIEN NV
		ON PDP.MANV = NV.MANV
GO
SELECT * FROM DS_PHIEU_DAT_PHONG
--STORED PROCEDURES
...
CREATE FUNCTION FN_KIEMTRATINHTRANGPHONG (
    @MAPHONG INT
) RETURNS NVARCHAR(20)
AS
BEGIN
    DECLARE @TINHTRANG NVARCHAR(20)
    SELECT @TINHTRANG = TINHTRANG FROM PHONG WHERE MAPHONG = @MAPHONG
    RETURN @TINHTRANG
END
...
@MaPhieuDatPhong int, @MaPhongCu int, @MaPhongMoi int, @NgayDoi nvarchar(100), 
@LyDo nvarchar(MAX)
AS
BEGIN
BEGIN TRAN
BEGIN TRY
    DECLARE @MADOITRA INT, @PHONGCU INT, @PHONGMOI INT
	  
	INSERT INTO DOITRAPHONG
	VALUES (@MaPhieuDatPhong,@MaPhongCu,@MaPhongMoi,@NgayDoi,@LyDo)
	
	SET @MADOITRA = (SELECT top 1 MADOITRA 
	FROM DOITRAPHONG
	order by MADOITRA DESC
	)

    DECLARE CURSOR_DOITRA CURSOR FOR 
    SELECT MADOITRA, PHONGCU, PHONGMOI FROM DOITRAPHONG WHERE MADOITRA = @MADOITRA

    OPEN CURSOR_DOITRA
    FETCH NEXT FROM CURSOR_DOITRA INTO @MADOITRA, @PHONGCU, @PHONGMOI

    WHILE @@FETCH_STATUS = 0
    BEGIN
        -- Update phòng cũ => Phòng trống
        UPDATE PHONG SET TINHTRANG = N'Phòng trống' WHERE MAPHONG = @PHONGCU

        -- Update phòng mới => Đã nhận
        UPDATE PHONG SET TINHTRANG = N'Đã nhận' WHERE MAPHONG = @PHONGMOI

        FETCH NEXT FROM CURSOR_DOITRA INTO @MADOITRA, @PHONGCU, @PHONGMOI
    END

    CLOSE CURSOR_DOITRA
    DEALLOCATE CURSOR_DOITRA
END TRY
BEGIN CATCH
    ROLLBACK TRAN
    RETURN
