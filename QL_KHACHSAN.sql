...
VALUES  ('12/12/2024','13/01/2025',1,1),
		('13/12/2024','15/01/2025',1,2),
		('12/12/2024','15/01/2025',1,3)
...
GO
--trigger:
CREATE TRIGGER TRG_CAPNHAT_TINHTRANG_DOITRA
ON DOITRAPHONG
AFTER INSERT
AS
BEGIN
    DECLARE @PHONGCU INT, @PHONGMOI INT

    SELECT @PHONGCU = PHONGCU, @PHONGMOI = PHONGMOI 
    FROM INSERTED
...
END
--View:
--View dùng để xem lịch sử phiếu đặt phòng
CREATE VIEW DS_PHIEU_DAT_PHONG AS
SELECT DISTINCT PDP.MAPHIEUDATPHONG, P.SOPHONG, KH.TENKH, PDP.NGAYLAP, NV.TENNV, NV.MANV, KH.MAKH
FROM PHIEUDATPHONG PDP
	JOIN CHITIETPHIEUDAT CTPD
		ON PDP.MAPHIEUDATPHONG = CTPD.MAPHIEUDATPHONG
	JOIN PHONG P
...
--CREATE PROCEDURE SP_DS_PHIEU_DAT_PHONG @SDT NVARCHAR(20), @MADD NVARCHAR(20)
--AS
--  SELECT DISTINCT PDP.MAPHIEUDATPHONG, P.SOPHONG, KH.TENKH, PDP.NGAYLAP, NV.TENNV
--  FROM PHIEUDATPHONG PDP
--	JOIN CHITIETPHIEUDAT CTPD
--		ON PDP.MAPHIEUDATPHONG = CTPD.MAPHIEUDATPHONG
--	JOIN PHONG P
...
--	JOIN NHANVIEN NV
--		ON PDP.MANV = NV.MANV
--	WHERE KH.SDT = @SDT OR KH.MADD = @MADD

--CREATE PROCEDURE SP_DSPHONG_TRONG_DOIPHONG @start_date nvarchar(50), @end_date nvarchar(50)
--AS 
--SELECT P.MAPHONG, P.SOPHONG, LP.TENLOAIPHONG, LP.GIA, LP.SOLUONGNGUOITOIDA
--FROM PHONG P
--INNER JOIN LOAIPHONG LP
--    ON P.MALOAIPHONG = LP.MALOAIPHONG
--WHERE P.MAPHONG NOT IN (
--    SELECT DISTINCT CTPD.MAPHONG
--    FROM CHITIETPHIEUDAT CTPD
--    INNER JOIN PHIEUDATPHONG PDP
--        ON CTPD.MAPHIEUDATPHONG = PDP.MAPHIEUDATPHONG
--    WHERE NOT (
--        PDP.NGAYDEN > @start_date OR PDP.NGAYTRADUKIEN < @end_date
--    )
--)


ALTER TABLE DOITRAPHONG
ADD CONSTRAINT CHK_PHONG_KHAC CHECK (PHONGCU <> PHONGMOI)

CREATE PROCEDURE SP_CAPNHAT_DOITRAPHONG
@MaPhieuDatPhong INT, 
@MaPhongCu INT, 
@MaPhongMoi INT, 
@NgayDoi NVARCHAR(100), 
@LyDo NVARCHAR(MAX), 
@MANV INT, 
@MAKH INT
AS
BEGIN
    BEGIN TRAN;
    BEGIN TRY
        DECLARE @MADOITRA INT, @MAPHIEUDATPHONG_MOI INT;

        -- Tạo phiếu đặt phòng mới
        INSERT INTO PHIEUDATPHONG (NGAYDEN, NGAYTRADUKIEN, MANV, MAKH)
        SELECT NGAYDEN, NGAYTRADUKIEN, MANV, MAKH
        FROM PHIEUDATPHONG
        WHERE MAPHIEUDATPHONG = @MaPhieuDatPhong;

        SET @MAPHIEUDATPHONG_MOI = SCOPE_IDENTITY();

        -- Tạo chi tiết phiếu đặt phòng mới
        INSERT INTO CHITIETPHIEUDAT (MAPHIEUDATPHONG, MAPHONG, NGAYTRAPHONG)
        VALUES (@MAPHIEUDATPHONG_MOI, @MaPhongMoi, NULL);

        IF EXISTS (
            SELECT 1 
            FROM CHITIETPHIEUDAT
            WHERE MAPHIEUDATPHONG = @MAPHIEUDATPHONG_MOI 
              AND MAPHONG = @MaPhongMoi
        )
        BEGIN
            -- Thêm vào DOITRAPHONG
            INSERT INTO DOITRAPHONG (MAPHIEUDATPHONG, PHONGCU, PHONGMOI, NGAYDOI, LYDO)
            VALUES (@MaPhieuDatPhong, @MaPhongCu, @MaPhongMoi, @NgayDoi, @LyDo);

            SET @MADOITRA = SCOPE_IDENTITY();
        END
        ELSE
        BEGIN
            RAISERROR ('Data inconsistency: MAPHIEUDATPHONG or PHONGCU does not exist in CHITIETPHIEUDAT.', 16, 1);
        END;

        INSERT INTO PHIEUNHANPHONG (MAPHIEUDATPHONG, MANV, NGAYNHAN)
        SELECT MAPHIEUDATPHONG, @MANV, GETDATE()
        FROM DOITRAPHONG
        WHERE MADOITRA = @MADOITRA;

		UPDATE PHIEUDICHVU
		SET MAPHIEUDATPHONG = @MAPHIEUDATPHONG_MOI
		WHERE MAPHIEUDATPHONG = @MaPhieuDatPhong
